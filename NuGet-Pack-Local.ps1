...
# AspNetCore
Invoke-Expression "dotnet pack .\Sources\Plea.AspNetCore\Plea.AspNetCore.csproj -p:PackageVersion=$($args[0]).$($args[1]).$($args[2]) --output $output"
Invoke-Expression "nuget add .\nupkgs\Plea.AspNetCore.$($args[0]).$($args[1]).$($args[2]).nupkg -source $source"

# Core
Invoke-Expression "dotnet pack .\Sources\Plea.Core\Plea.Core.csproj -p:PackageVersion=$($args[0]).$($args[1]).$($args[2]) --output $output"
